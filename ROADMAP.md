...
| Area | State |
|------|-------|
| Rendering | Complete. Four Metal compute passes per frame (floor/ceiling, DDA walls + z-buffer, z-tested sprites + weapon overlay, screen effects + nearest-neighbour upscale) presented straight into an `MTKView` with three frames in flight and no CPU readback; the view's display link drives the game loop. A multi-core CPU raycaster with CPU sprite compositing remains as the fallback when Metal is unavailable. |
